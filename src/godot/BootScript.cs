...
public partial class BootScript : Node2D
{
	...
	private void Preload(Configuration exam, string imgDir, List<TrialOrder> list)
	{
        imgDir = Path.Combine(imgDir, "Stimulus images");
		...
		foreach (var itm in drawItems)
		{
			...
			switch (itm.itemType)
			{
				case PlayItemType.Text:
					...
					{
						DrawString(fnt, pos, itm.text, HorizontalAlignment.Center);
					} 
					else
					{
